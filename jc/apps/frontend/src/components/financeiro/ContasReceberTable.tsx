...
interface ContasReceberTableProps {
  ...
  onView?: (conta: ContaReceber) => void;
}

export default function ContasReceberTable({ 
  contas, 
  searchTerm, 
  setSearchTerm, 
  filterStatus, 
            ...
            <tbody>
              {paginatedContas.map((conta) => {
                ...
                return (
                  <tr key={conta.id}>
                    <td>
                      <div className="fw-bold">{conta.cliente}</div>
                      <small className="text-muted">NIF: {conta.nifCliente}</small>
                    </td>
                    <td>
